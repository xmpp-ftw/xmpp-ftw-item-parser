...
var parseIodefDocument = function(iodefXml, entity) {

    entity['IODEF-Document'] = {}

    var iodefDocumentAttributes = ['version', 'lang', 'formatid']
    iodefDocumentAttributes.forEach(function(attribute) {
        if (iodefXml.attrs[attribute]) entity['IODEF-Document'][attribute] = iodefXml.attrs[attribute]
    })

    parseIncidents(iodefXml, entity['IODEF-Document'])
}
...
exports.parse = function(iodefXml, entity) {

    if (!iodefXml.is('IODEF-Document', NS_IODEF)) {
        iodefXml = iodefXml.getChild('IODEF-Document', NS_IODEF)
    }

    if (!iodefXml) return

    if (VERSION_IODEF !== iodefXml.attrs.version) return

    parseIodefDocument(iodefXml, entity)
}
