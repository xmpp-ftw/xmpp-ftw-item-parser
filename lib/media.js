...
var NS_PREFIX_MEDIA = exports.NS_PREFIX_MEDIA = 'bcm'
var NS_ATOM = exports.NS_ATOM = require('./atom').NS_ATOM

exports.parse = function(item, entity) {
    var entry = item.getChild('entry')
    if (!entry || (entry.attrs['xmlns:' + NS_PREFIX_MEDIA] !== NS_BUDDYCLOUD_MEDIA)) return
    ...
    media.getChildren('item').forEach(function(item) {
        entity.media.push(item.attr('id').trim())
    }, this)
}
...
exports.build = function(data, stanza) {
    ...
    stanza.getChild('entry', NS_ATOM).attr('xmlns:' + NS_PREFIX_MEDIA, NS_BUDDYCLOUD_MEDIA)
    var media = stanza.getChild('entry').c(NS_PREFIX_MEDIA + ':media')
    data.media.forEach(function(item) {
        media.c(NS_PREFIX_MEDIA + ':item', { id: item })
    }, this)
}
